...
export const bookingConfirm = createAsyncThunk(
  "booking/confirm",
  async (id, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        getErrorMessage(error, "Booking confirmation failed"),
      );
    }
  },
);
...
export const bookingCancel = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        getErrorMessage(error, "Booking cancellation failed"),
      );
    }
  },
);
...
export const lessonStart = createAsyncThunk(
  "booking/lessonStart",
  async (id, { rejectWithValue }) => {
    try {
...
export const attendanceConfirm = createAsyncThunk(
  ...
  async ({ id, formData }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        getErrorMessage(error, "Attendance confirmation failed"),
      );
    }
  },
);

export const lessonComplete = createAsyncThunk(
  "booking/lessonComplete",
  async (id, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        getErrorMessage(error, "Failed to complete lesson"),
      );
    }
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchLessons.fulfilled, (state, action) => {
        state.loading = false;
        const payload = action.payload?.data || action.payload;
        state.lessons = Array.isArray(payload)
          ? payload
          : payload?.lessons || [];
      })
      ...
      .addCase(fetchPayments.fulfilled, (state, action) => {
        state.loading = false;
        const payload = action.payload?.data || action.payload;
        state.payments = Array.isArray(payload)
          ? payload
          : payload?.payments || [];
      })
      .addCase(fetchInvoices.fulfilled, (state, action) => {
        state.loading = false;
        const payload = action.payload?.data || action.payload;
        state.invoices = Array.isArray(payload)
          ? payload
          : payload?.invoices || [];
      })
