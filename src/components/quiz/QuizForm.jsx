...
export default function QuizForm({
  ...
  const previewUrl = useMemo(() => {
    ...
    return !removeCoverImage && initialValues?.coverImage
      ? mediaUrl(initialValues.coverImage)
      : "";
    ...
    >
      <section className="p-5 sm:p-6">
        <div className="mb-5">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Name the quiz and choose where it will appear.
          </p>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Example: Simple Series 01"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {initialValues?.type === "road_sign" ? (
                <div className="rounded-xl border border-[#173f87] bg-blue-50 px-4 py-3 text-sm font-bold text-[#173f87]">
                  Road Sign (Legacy)
                </div>
              ) : (
                quizTypes.map((item) => (
                  <button
                    ...
                    key={item.value}
                    onClick={() =>
                      setForm((current) => ({ ...current, type: item.value }))
                    }
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="What will students practise in this quiz?"
              className="min-h-[100px] w-full resize-y rounded-xl border border-slate-200 bg-[#f8f8fb] px-3 py-3 text-sm leading-6 text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-300 focus:bg-white focus:ring-4 focus:ring-blue-50"
            />
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-slate-100 bg-[#fbfcfe] p-5 sm:p-6">
        <div className="mb-5">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Set the time, pass mark and publishing options.
          </p>
        </div>
          ...
          <div>
            ...
            <label className="flex h-11 cursor-pointer items-center justify-between rounded-xl border border-slate-200 bg-white px-3">
              <span className="text-xs font-bold text-slate-700">
                {form.isPaid ? "Paid quiz" : "Free quiz"}
              </span>
              <input
                type="checkbox"
                name="isPaid"
                checked={form.isPaid}
                onChange={handleChange}
                className="h-4 w-4 accent-[#173f87]"
              />
            </label>
          </div>
        </div>
      ...
      <section className="border-t border-slate-100 p-5 sm:p-6">
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Optional. Use a clear landscape JPG, PNG or WEBP image.
          </p>
        </div>
          ...
          <div className="flex h-[110px] items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
            {previewUrl ? (
              ...
              <div className="text-center">
                ...
                <p className="text-[11px] font-bold text-slate-400">Preview</p>
              </div>
            )}
          </div>
        </div>
        {initialValues?.coverImage && !coverImage && (
          <button
            ...
            className={`mt-3 text-xs font-bold ${removeCoverImage ? "text-blue-700" : "text-red-600"}`}
          >
            {removeCoverImage
              ? "Undo image removal"
      ...
      <footer className="flex flex-col justify-between gap-3 border-t border-slate-200 bg-[#fbfcfe] px-5 py-4 sm:flex-row sm:items-center sm:px-6">
        ...
        <button
          type="submit"
          disabled={loading}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#173f87] px-6 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#12346f] disabled:cursor-not-allowed disabled:opacity-60"
        >
          <FaSave />
