...
function Editor() {
  ...
  const submit = async (event) => {
    ...
    const blockMeta = blocks.map((block) => { const item = { ...block, bulletPoints: (block.bulletPoints || []).map((point) => point.trim()).filter(Boolean) }; delete item.file; if (block.file) { item.fileIndex = data.getAll("blockImages").length; data.append("blockImages", block.file); } return item; });
...
function BulletEditor({ blockIndex, value, onChange }) {
  const addAfter = (index = value.length - 1) => {
    ...
    onChange(next);
    setTimeout(() => document.getElementById(`bullet-${blockIndex}-${index + 1}`)?.focus(), 0);
  };
  return <div className="mt-3">
    <div className="flex items-center justify-between gap-3">
      <div><p className="text-xs font-bold">Bullet points</p><p className="mt-1 text-[11px] font-normal text-slate-500">Press Enter to add the next point.</p></div>
      <button type="button" onClick={() => addAfter()} className="shrink-0 rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">+ Add point</button>
    </div>
    <div className="mt-2 space-y-2">
      {value.map((point, index) => <div key={index} className="flex items-center gap-2">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#e5ebf5] text-[11px] font-bold text-[#173f87]">{index + 1}</span>
        <input id={`bullet-${blockIndex}-${index}`} value={point} placeholder="Write a bullet point" onChange={(event) => onChange(value.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addAfter(index); } }} className="min-w-0 flex-1 rounded-lg border bg-white px-3 py-2.5 text-sm font-normal"/>
        <button type="button" aria-label={`Remove bullet ${index + 1}`} onClick={() => onChange(value.filter((_, itemIndex) => itemIndex !== index))} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-red-50 text-lg font-bold text-red-600">×</button>
      </div>)}
      {!value.length && <button type="button" onClick={() => addAfter()} className="w-full rounded-lg border border-dashed border-slate-300 bg-white px-4 py-4 text-xs font-semibold text-slate-500">No bullet points. Click to add the first one.</button>}
    </div>
  </div>;
}
