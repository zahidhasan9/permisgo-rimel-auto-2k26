...
//     blogLabel: "Blog",
//     blogTitle: "News and Insights",
//     readMore: "Read More",
//     viewBlogs: "View All Blogs",
//     serviceTitles: [
//       "Manual category B licence",
//       "Automatic category B licence",
//       "Accompanied driving",
//       "Supervised driving (AAC)",
//       "Classroom Highway Code",
//       "Online Highway Code",
//       "Supervised driving",
//       "Accelerated licence",
...
//     blogLabel: "ব্লগ",
//     blogTitle: "সংবাদ ও গুরুত্বপূর্ণ তথ্য",
//     readMore: "আরও পড়ুন",
//     viewBlogs: "সব ব্লগ দেখুন",
//     serviceTitles: [
//       "ম্যানুয়াল ক্যাটাগরি বি লাইসেন্স",
//       "অটোমেটিক ক্যাটাগরি বি লাইসেন্স",
//       "সহযোগী ড্রাইভিং",
//       "তত্ত্বাবধানে ড্রাইভিং (AAC)",
//       "ক্লাসরুম হাইওয়ে কোড",
//       "অনলাইন হাইওয়ে কোড",
//       "তত্ত্বাবধানে ড্রাইভিং",
//       "দ্রুত লাইসেন্স কোর্স",
//       "দ্রুত হাইওয়ে কোড",
//     ],
//   },
//   fr: {
...
//     blogLabel: "Blog",
//     blogTitle: "Actualités et conseils",
//     readMore: "Lire la suite",
//     viewBlogs: "Voir tous les articles",
//     serviceTitles: [
//       "Permis B conduite manuelle",
//       "Permis B conduite automatique",
//       "Conduite accompagnée",
//       "Conduite supervisée (AAC)",
//       "Code en salle",
//       "Code en ligne",
//       "Conduite supervisée",
//       "Permis accéléré",
...
//     try {
//       await createContactSubmission(contactForm);
//       setContactForm({
//         firstName: "",
//         lastName: "",
//         email: "",
...
//     } catch (error) {
//       toast.error(
//         error.response?.data?.message || "Your message could not be sent.",
//       );
//     } finally {
...
//       .then(({ data }) => {
//         if (active) setHomeBlogs(data?.data || []);
//       })
//       .catch(() => {
//         if (active) setHomeBlogs([]);
//       });
//     return () => {
//       active = false;
//     };
...
//       })
//       .finally(() => {
//         if (active) setHomeFaqLoading(false);
//       });
//     return () => {
//       active = false;
//     };
...
//                         <p
//                           data-no-translate
//                           className="mt-1 text-[11px] font-bold text-[#27d26b] line-through"
//                         >
...
//                         <h3
//                           data-no-translate
//                           className="mt-1 text-[22px] font-black leading-none text-white"
//                         >
...
//                           type="button"
//                           onClick={() => setSelectedNearbyTeacher(teacher)}
//                           className={cn(
//                             "flex w-full items-center gap-3 rounded-[8px] border bg-white p-2 text-left transition",
//                             selectedNearbyTeacher?.user?._id ===
//                               teacher.user?._id
//                               ? "border-[#174FA5] ring-2 ring-[#174FA5]/10"
//                               : "border-transparent hover:border-[#b8c9e5]",
//                           )}
//                         >
//                           {teacher.user?.avatar ? (
//                             <img
...
//                             />
//                           ) : (
//                             <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#d9e5f4] text-xs font-black text-[#174fa5]">
//                               {(teacher.user?.name || "I").charAt(0)}
//                             </span>
//                           )}
//                           <span className="min-w-0">
//                             <span className="block truncate text-xs font-extrabold text-[#174fa5]">
//                               {teacher.user?.name || "Driving Instructor"}
//                             </span>
//                             <span className="block truncate text-[10px] text-slate-500">
//                               {teacher.locations?.[0]?.address ||
//                                 teacher.locations?.[0]?.city ||
//                                 "Location available"}
//                             </span>
//                           </span>
//                         </button>
//                       ))}
//                       {!nearbyTeachers.length && !teacherSearchLoading && (
//                         <p className="rounded-lg bg-white p-3 text-center text-xs text-slate-500">
...
//                         {selectedNearbyTeacher.user?.avatar ? (
//                           <img
//                             src={mediaUrl(selectedNearbyTeacher.user.avatar)}
//                             alt={
//                               selectedNearbyTeacher.user?.name || "Instructor"
//                             }
...
//                             {(selectedNearbyTeacher.user?.name || "I").charAt(
//                               0,
//                             )}
//                           </div>
//                         )}
...
//                             {selectedNearbyTeacher.user?.name ||
//                               "Driving Instructor"}
//                           </h4>
//                         </div>
//                       </div>
...
//                             Experience{" "}
//                             {Number(selectedNearbyTeacher.experienceYears || 0)}{" "}
//                             Years+
//                           </p>
...
//                       </p>
//                       <p className="text-[10px] font-semibold text-[#333840]">
//                         {selectedNearbyTeacher.locations?.[0]?.address ||
//                           selectedNearbyTeacher.locations?.[0]?.city ||
//                           "Location available during booking"}
//                       </p>
//                       <p className="mt-2 text-[10px] font-semibold capitalize text-[#333840]">
//                         {selectedNearbyTeacher.vehicles
//                           ?.map((vehicle) => vehicle.vehicleType)
//                           .filter(
//                             (value, index, list) =>
//                               list.indexOf(value) === index,
//                           )
//                           .join(" · ") || "Vehicle available"}
//                       </p>
//                     </div>
...
//                         />
//                       ) : (
//                         <span className="flex h-full w-full items-center justify-center bg-[#d9e5f4] text-lg font-black text-[#174fa5]">
...
//                             key={star}
//                             className={
//                               star < Math.round(rating) ? "" : "text-slate-300"
//                             }
//                           />
//                         ))}
//                       </div>
...
//                         onClick={() =>
//                           toast.info(
//                             "Please message your instructor from the Student Dashboard.",
//                           )
//                         }
...
//             })}
//           </Swiper>
//           {homeInstructorsLoading && (
//             <p className="py-12 text-center text-sm font-semibold text-slate-500">
//               Loading instructors...
//             </p>
//           )}
//           {!homeInstructorsLoading && !homeInstructors.length && (
//             <p className="rounded-xl bg-[#e7ecf4] py-12 text-center text-sm font-semibold text-slate-500">
...
//                         className="h-[38px] w-full rounded-[8px] border border-[#C8D1DD] bg-white px-[13px] text-[10px] outline-none"
//                       />
//                     </div>
//                     <div>
//                       <label
...
//               {homeFaqLoading && (
//                 <p className="rounded-[8px] bg-[#F4F6F9] p-6 text-center text-sm text-[#4F555E]">
//                   Loading FAQs...
//                 </p>
//               )}
//               {!homeFaqLoading && homeFaqError && (
//                 <p className="rounded-[8px] bg-red-50 p-6 text-center text-sm text-red-600">
//                   {homeFaqError}
//                 </p>
//               )}
//               {!homeFaqLoading && !homeFaqError && homeFaqs.length === 0 && (
//                 <p className="rounded-[8px] bg-[#F4F6F9] p-6 text-center text-sm text-[#4F555E]">
        ...
        <div className={container}>
          <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-12">
            <div className="lg:col-span-8">
              <div className="mb-6 max-w-xl">
                <h5 className={badge}>
                  {homeCmsText("paymentLabel", "Payment System")}
                </h5>
