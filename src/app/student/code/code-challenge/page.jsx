...
function QuizContent() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#111] text-[#171717] sm:bg-white sm:px-6 sm:py-6">
      <div
        ref={panelRef}
        className="mx-auto min-h-screen w-full max-w-[1084px] bg-[#111] sm:min-h-0 sm:bg-white"
      >
        <header className="flex h-11 items-center justify-between bg-[#303435] px-4 text-white sm:hidden">
          <p className="text-xs font-semibold">
            Question <span className="text-[#27a9c7]">{currentIndex + 1}</span>{" "}
            / {questions.length}
          </p>
        ...
        <section className="min-h-[calc(100dvh-44px)] bg-[#303435] pb-[52px] sm:min-h-[760px] sm:rounded-xl sm:bg-[#e8eef7] sm:p-6">
          ...
          {feedback && (
            <div className="mx-1 mb-2 flex h-7 text-[12px] sm:hidden">
              ...
              <span
                className={`flex flex-1 items-center justify-center text-white ${feedback.isCorrect ? "bg-[#20c23b]" : "bg-[#ef3038]"}`}
              >
                      ...
                      .map((option, localIndex) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={`shrink-0 text-[12px] sm:min-w-8 sm:text-left sm:text-[16px] sm:font-medium sm:text-slate-900 ${selectedIndexes.includes(index) ? "text-[#27b9c8]" : "text-white"}`}
                            >
                              {letter(index)}
                            </span>
                          </div>
