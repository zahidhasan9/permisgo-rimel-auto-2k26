      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[265px_minmax(0,1fr)]">
        ...
        <section className="space-y-7">
          ...
          {visibleAnswers.map((answer) => {
            ...
            const selectedIndexes = (answer.selectedOptionIndexes?.length ? answer.selectedOptionIndexes : [answer.selectedOptionIndex]).map(Number);
            const correctIndexes = (answer.correctOptionIndexes?.length ? answer.correctOptionIndexes : [answer.correctOptionIndex]).map(Number);
            const isMultiPrompt = Number(question.promptCount) === 2;
            const promptGroups = isMultiPrompt
              ? [
                { label: "Question 1", text: question.questionText, indexes: [0, 1] },
                { label: "Question 2", text: question.secondaryQuestionText, indexes: [2, 3] },
              ]
              : [{ label: "Question", text: question.questionText, indexes: [0, 1, 2, 3] }];
            const groupResults = promptGroups.map((group) => {
              const selectedIndex = selectedIndexes.find((index) => group.indexes.includes(index));
              const correctIndex = correctIndexes.find((index) => group.indexes.includes(index));
              return { ...group, selectedIndex, correctIndex, isCorrect: selectedIndex === correctIndex };
            });
            ...
            return <article ref={(node) => { refs.current[originalIndex] = node; }} key={`${question._id}-${originalIndex}`} className="scroll-mt-5 overflow-hidden rounded-[11px] bg-[#e8eef7] p-3">
              {videoEmbedUrl ? <iframe src={videoEmbedUrl} title="Question video" className="aspect-video w-full rounded-[9px] bg-black" allow="encrypted-media; picture-in-picture" allowFullScreen/> : question.questionVideoUrl ? <video src={question.questionVideoUrl} controls className="max-h-[365px] w-full rounded-[9px] bg-black"/> : question.questionImage && <img src={mediaUrl(question.questionImage)} alt="Question" className="h-[250px] w-full rounded-[9px] bg-white object-cover sm:h-[365px]"/>}
              <div className={`mt-5 grid gap-5 ${answer.isCorrect ? "lg:grid-cols-[minmax(0,1fr)_210px]" : "lg:grid-cols-[minmax(0,1fr)_230px]"}`}>
                <div className="min-w-0">
                  ...
                  <div className="mt-7 space-y-4">{promptGroups.map((group) => <section key={group.label} className={isMultiPrompt ? "rounded-lg border border-white/80 bg-white/35 p-3" : ""}>
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="text-[13px] font-bold">{isMultiPrompt && <span className="mr-1 text-[#173f87]">{group.label}:</span>}{group.text}</h3>
                      {isMultiPrompt && <span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-bold text-white ${groupResults.find((item) => item.label === group.label)?.isCorrect ? "bg-[#20bd39]" : "bg-[#e2253b]"}`}>{groupResults.find((item) => item.label === group.label)?.isCorrect ? "Correct" : "Wrong"}</span>}
                    </div>
                    <div className="mt-2 space-y-2">{question.options?.map((option, index) => {
                      if (!group.indexes.includes(index)) return null;
                      const correctOption = correctIndexes.includes(index);
                      const wrongSelection = selectedIndexes.includes(index) && !correctOption;
                      return <div key={index} className={`flex min-h-[38px] items-center justify-between gap-4 rounded-[7px] border px-3 py-2 text-[11px] ${correctOption ? "border-[#20bd39] bg-[#f5fff6] text-[#287b35]" : wrongSelection ? "border-[#e2253b] bg-[#fff7f8] text-[#b82033]" : "border-transparent bg-transparent text-slate-600"}`}><span>{option.text}</span><span className="min-w-[40px] text-right font-bold">{letter(index)}</span></div>;
                    })}</div>
                  </section>)}</div>
                </div>
                <aside className="space-y-3">
                  ...
                  <div className="rounded-[9px] bg-white p-4 text-[11px]">
                    {groupResults.map((group) => <div key={group.label} className="not-first:mt-4">
                      <p className="font-bold text-[#173f87]">{isMultiPrompt ? group.label : "Your answer"}</p>
                      <div className="mt-2 flex flex-wrap gap-2">
                        <span className={`rounded-[6px] px-3 py-2 font-bold text-white ${group.isCorrect ? "bg-[#20bd39]" : "bg-[#e2253b]"}`}>Selected: {group.selectedIndex === undefined ? "—" : letter(group.selectedIndex)}</span>
                        {!group.isCorrect && <span className="rounded-[6px] bg-[#20bd39] px-3 py-2 font-bold text-white">Correct: {group.correctIndex === undefined ? "—" : letter(group.correctIndex)}</span>}
                      </div>
                    </div>)}
                  </div>
                </aside>
