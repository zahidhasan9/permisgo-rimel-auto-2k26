  ...
  return (
    <main className="min-h-screen bg-white px-3 py-6 sm:px-6">
      <div className="mx-auto w-full max-w-[1084px]">
        ...
        {error && (
          <p className="mt-8 rounded-xl border border-red-200 bg-red-50 p-5 text-sm font-semibold text-red-700">
            {error}
          </p>
        )}
        {loading ? (
          <section className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-[210px] animate-pulse rounded-xl bg-[#e8eef7]"
              />
            ))}
          </section>
        ) : (
          <section className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map((course) => (
              <Link
                key={course._id}
                href={`/student/code/code-ebook/${course._id}`}
                className="group overflow-hidden rounded-xl bg-[#e8eef7] p-4 transition hover:-translate-y-1 hover:shadow-lg"
              >
                ...
                <div className="flex min-h-[92px] items-center justify-between gap-3 px-2 pt-4">
                  <div>
                    <h2 className="font-bold leading-5 text-[#173f87]">
                      {course.title}
                    </h2>
                  ...
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white text-[#173f87]">
                    <IoChevronForward />
                  </span>
                </div>
              </Link>
        ...
        {!loading && !error && !courses.length && (
          <div className="mt-8 rounded-xl bg-[#e8eef7] p-12 text-center text-sm text-slate-500">
            No active Code eBook course is available.
          </div>
        )}
      </div>
    </main>
