...
export default function KnowledgeSheetsPage() {
  ...
  useEffect(() => {
    getLearningContents({ type: "knowledge-sheet" })
      ...
      .catch((e) =>
        setError(
          e.response?.data?.message || "Knowledge sheets could not be loaded.",
        ),
      )
  ...
  const download = async (sheet) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `${sheet.title || "knowledge-sheet"}.pdf`;
          ...
          {loading ? (
            <div className="grid gap-3 md:grid-cols-2">
              {Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[91px] animate-pulse rounded-xl bg-white"
                />
              ))}
            </div>
          ) : sheets.length ? (
            <div className="grid gap-3 md:grid-cols-2">
              {sheets.map((sheet) => (
                <article
                  key={sheet._id}
                  className="flex min-w-0 flex-col gap-4 rounded-xl bg-white p-4 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div className="min-w-0">
                    <h2 className="break-words text-base font-bold leading-6 text-black">
                      {sheet.title}
                    </h2>
                    {sheet.description && (
                    ...
                    <p className="mt-2 flex items-center gap-1.5 text-sm text-slate-500">
                      ...
                      {sheet.readMinutes || 0} minutes read
                    </p>
                  </div>
                  {sheet.fileUrl ? (
                    <button
                      type="button"
                      disabled={downloading === sheet._id}
                      onClick={() => download(sheet)}
                      className="flex h-9 w-full shrink-0 items-center justify-center rounded-lg border border-[#174a9b] px-6 text-xs font-bold text-[#174a9b] hover:bg-[#174a9b] hover:text-white disabled:opacity-50 sm:w-[125px]"
                    >
                      {downloading === sheet._id
                        ? "Downloading..."
                        : "Download"}
                    </button>
                  ) : (
                    <span className="flex h-9 w-full shrink-0 items-center justify-center rounded-lg border border-slate-200 px-6 text-xs font-bold text-slate-400 sm:w-[125px]">
                      Unavailable
                    </span>
                  )}
                </article>
            ...
            <p className="rounded-xl bg-white p-10 text-center text-slate-500">
              No knowledge sheets available yet.
            </p>
          )}
        </section>
      </div>
