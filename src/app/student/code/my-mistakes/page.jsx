...
export default function MyMistakesPage() {
  ...
  if (loading) {
    return <main className="min-h-screen bg-white p-6"><div className="mx-auto h-[720px] w-full animate-pulse rounded-xl bg-[#e8eef7]" /></main>;
  }
    ...
    <main className="min-h-screen bg-white px-3 py-6 text-[#171717] sm:px-6">
      <div ref={panelRef} className="mx-auto w-full max-w-[1084px]">
        <header className="mb-8 flex h-11 items-center gap-4">
          ...
          <div>
            <h1 className="text-[25px] font-semibold leading-none text-[#173f87]">Code Challenge</h1>
            <p className="mt-2 text-xs font-medium text-slate-500">My mistakes · Read-only review</p>
          </div>
        </header>

        {error && <div className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm font-semibold text-red-700">{error}</div>}

        {!error && !item && (
          <section className="rounded-xl bg-[#e8eef7] p-12 text-center">
            <h2 className="text-xl font-bold text-[#173f87]">No mistakes found</h2>
            <p className="mt-2 text-sm text-slate-500">Wrong answers from completed quizzes will appear here.</p>
            <Link href="/student/code/simple-series-list" className="mt-5 inline-flex rounded-lg bg-[#173f87] px-5 py-2.5 text-xs font-bold text-white">Take a Quiz</Link>
          </section>
        )}
        ...
        {!error && item && (
          <section className="min-h-[720px] rounded-xl bg-[#e8eef7] p-4 sm:p-6">
            <div className="mb-6 flex items-center justify-between text-[#123f88]">
              <div>
                <p className="text-[16px] font-bold">Question {index + 1}/{mistakes.length}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500">{item.quizTitle}</p>
              </div>
              <div className="flex items-center gap-4">
                <button type="button" onClick={() => !document.fullscreenElement ? panelRef.current?.requestFullscreen?.() : document.exitFullscreen?.()} title="Fullscreen"><FaExpandArrowsAlt size={22} /></button>
                <button type="button" onClick={speak} title="Read question">{speaking ? <IoVolumeMute size={28} /> : <IoVolumeHigh size={28} />}</button>
              </div>
            </div>
            ...
            <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_330px]">
              <div>
                ...
                {question.explanationText && <p className="mt-5 rounded-lg bg-white/80 p-4 text-sm leading-6 text-[#123f88]"><b>Explanation: </b>{question.explanationText}</p>}
              </div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-slate-300 pt-5">
              <button type="button" disabled={index === 0} onClick={() => move(index - 1)} className="flex h-11 items-center gap-2 rounded-xl bg-white px-5 text-sm font-bold text-[#123f88] disabled:cursor-not-allowed disabled:opacity-40"><IoArrowBack /> Previous</button>
              <p className="hidden text-xs font-semibold text-slate-500 sm:block">Review only — answers cannot be changed</p>
              <button type="button" disabled={index + 1 >= mistakes.length} onClick={() => move(index + 1)} className="flex h-11 items-center gap-2 rounded-xl bg-[#e3263c] px-5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Next <IoArrowForward /></button>
            </div>
          </section>
        )}
      </div>
