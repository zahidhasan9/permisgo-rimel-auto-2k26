...
export default function PlanningPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-white px-2.5 pb-24 pt-3 sm:p-5 sm:pb-8">
      <h1 className="text-lg font-extrabold text-blue-900 sm:text-xl">Planning</h1>
      ...
      <section className="mt-3 rounded-xl bg-[#e8eef7] p-2.5 sm:mt-5 sm:rounded-2xl sm:p-5">
        <div className="flex min-w-0 items-center justify-between gap-2 rounded-xl bg-white px-3 py-2.5 sm:px-4 sm:py-3">
          <strong className="min-w-0 text-[12px] text-slate-800 sm:text-base">{rangeLabel}</strong>
          <div className="flex gap-2 text-blue-900">
            <button type="button" aria-label="Previous week" onClick={() => setWeekStart((current) => addDays(current, -7))} className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-sm sm:h-10 sm:w-10"><FaChevronLeft /></button>
            <button type="button" aria-label="Next week" onClick={() => setWeekStart((current) => addDays(current, 7))} className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-sm sm:h-10 sm:w-10"><FaChevronRight /></button>
          </div>
        </div>
        ...
        <div className="mt-2.5 md:hidden">
          {loading ? (
            <div className="space-y-2.5">
              {[1, 2, 3].map((item) => <div key={item} className="h-24 animate-pulse rounded-xl bg-white" />)}
            </div>
          ) : mobileSchedule.length ? (
            <div className="space-y-3">
              {mobileSchedule.map(({ day, lessons: dayLessons }) => (
                <section key={dateKey(day)} className="overflow-hidden rounded-xl bg-white">
                  <header className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-3 py-2.5">
                    <div>
                      <p className="text-[11px] font-bold uppercase tracking-wide text-[#174a9b]">{day.toLocaleDateString("en-US", { weekday: "long" })}</p>
                      <p className="mt-0.5 text-[10px] text-slate-500">{day.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}</p>
                    </div>
                    <span className="rounded-full bg-[#e8eef7] px-2.5 py-1 text-[10px] font-bold text-[#174a9b]">{dayLessons.length} {dayLessons.length === 1 ? "lesson" : "lessons"}</span>
                  </header>
                  <div className="divide-y divide-slate-100">
                    {dayLessons.map((lesson) => (
                      <button type="button" key={lesson._id} onClick={() => router.push(`/student/lessons/${lesson._id}`)} className="flex w-full min-w-0 items-center gap-3 px-3 py-3 text-left active:bg-slate-50">
                        <div className="flex w-[74px] shrink-0 flex-col items-center rounded-lg bg-[#174a9b] px-2 py-2 text-white">
                          <span className="text-[11px] font-bold">{formatTime(lesson.startTime)}</span>
                          <span className="mt-1 text-[9px] opacity-80">to {formatTime(lesson.endTime)}</span>
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-bold text-slate-800">Booked lesson</p>
                          <p className="mt-1 truncate text-[11px] font-semibold text-[#174a9b]">{lesson.teacher?.name || "Instructor"}</p>
                          <p className="mt-1 truncate text-[10px] text-slate-500">{lesson.booking?.location?.address || lesson.location?.address || "Lesson location"}</p>
                        </div>
            ...
            <div className="rounded-xl bg-white px-4 py-10 text-center text-xs font-semibold text-slate-500">No booked lesson in this week.</div>
          )}
        </div>
