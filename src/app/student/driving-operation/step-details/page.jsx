        ...
        if (mounted)
          setError(
            requestError?.response?.data?.message ||
              "Journey details could not be loaded.",
          );
      })
      .finally(() => {
          ...
          <div className="relative mt-[20px]">
            ...
            <div className="space-y-[22px]">
              {steps.map((step, index) => {
                const active =
                  step.code === "driving_training" && !step.completed;
                return (
                  <article key={step.code} className="relative flex gap-[16px]">
                    <div className="relative z-10 flex w-[32px] shrink-0 justify-center">
                      <div
                        className={`flex h-[31px] w-[31px] items-center justify-center rounded-full text-[18px] ${step.completed ? "bg-[#174A9B] text-white" : active ? "bg-white text-[#174A9B] shadow" : "bg-white text-transparent"}`}
                      >
                        {step.completed ? (
                          <IoCheckmark />
                        ) : active ? (
                          <FaCarSide className="text-[16px]" />
                        ) : null}
                      </div>
                    </div>

                    <div
                      className={`min-w-0 flex-1 rounded-[12px] px-[15px] ${active ? "min-h-[196px] bg-white py-[15px]" : "min-h-[69px] bg-[#DDE5F1] py-[15px]"}`}
                    >
                      <p
                        className={`text-[14px] font-[500] leading-[22px] ${active ? "w-fit rounded-[4px] bg-[#EEF2FA] px-[7px] py-[3px] font-[700] text-[#174A9B]" : "text-[#62656B]"}`}
                      >
                        Step {String(index + 1).padStart(2, "0")} :
                      </p>
                      <h2
                        className={`${active ? "mt-[9px]" : "mt-[1px]"} text-[15px] font-[700] leading-[19px] text-[#171717]`}
                      >
                        {step.label}
                      </h2>
                      ...
                      {step.description && (
                        <p
                          className={`${active ? "mt-[5px]" : "mt-[7px]"} text-[13px] font-[400] leading-[19px] text-[#666A72]`}
                        >
                          {step.description}
                        </p>
                      )}
                      ...
                      {active && step.code === "driving_training" && (
                        <div className="mt-[14px] grid grid-cols-1 gap-[18px] md:grid-cols-3">
                          <TrainingStatus
                            ...
                            label="Driving hours"
                            value={`${journey?.completedHours || 0}h/${journey?.targetHours || 20}h`}
                          />
                          <TrainingStatus
                            href="/student/profile/booklet"
                            label="Skills acquired"
                            value={`${journey?.skillsPercentage || 0}%/${journey?.targetSkillsPercentage || 60}%`}
                          />
                          <TrainingStatus
...
function TrainingStatus({ href, label, value, muted = false }) {
  return (
    <div className="flex h-[69px] items-center justify-between rounded-[10px] bg-[#DDE5F1] px-[15px]">
      <div>
        <p className="text-[14px] font-[500] leading-[18px] text-[#656A72]">
          {label}
        </p>
        <Link
          href={href}
          className="mt-[2px] block text-[13px] font-[700] leading-[17px] text-[#174A9B] underline underline-offset-[1px]"
        >
          See Details
        </Link>
      </div>
      <span
        className={`flex h-[41px] min-w-[42px] items-center justify-center rounded-[5px] px-[9px] text-[14px] font-[700] ${muted ? "bg-white text-[#555]" : "bg-[#3478DF] text-white"}`}
      >
        {value}
      </span>
    </div>
