...
function PostMeta({ post }) {
  return <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[12px] font-medium text-slate-700 sm:text-[13px]">
    <span className="flex items-center gap-1.5"><FaUser className="text-[#174a9b]" /> {post.author?.name || "PermisGo"}</span>
    <span className="flex items-center gap-1.5"><FaCalendarAlt className="text-[#174a9b]" /> {formatDate(post.publishedAt || post.createdAt)}</span>
    <span className="flex items-center gap-1.5"><FaFolderOpen className="text-[#174a9b]" /> Driving school</span>
    <span className="flex items-center gap-1.5"><FaCommentAlt className="text-[#174a9b]" /> No comments</span>
  </div>;
}
...
export default function BlogsPage() {
  ...
  return <section className="bg-white px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-[72px]">
    <div className="mx-auto grid max-w-[1180px] items-start gap-6 lg:grid-cols-[minmax(0,1fr)_270px]">
      ...
      <aside className="rounded-[12px] bg-[#dfe7f3] p-4 lg:sticky lg:top-5"><h2 className="rounded-[8px] bg-[#174a9b] px-4 py-3 text-center text-[15px] font-extrabold text-white">Our Recent Blog Post</h2><div className="mt-4 space-y-3">{recentPosts.map((post) => <Link key={post._id} href={`/blogs/${post.slug}`} className="group grid grid-cols-[76px_1fr] items-center gap-3 rounded-[9px] bg-white p-2.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="relative h-[56px] overflow-hidden rounded-[7px]"><Image src={post.coverImage} alt="" fill sizes="76px" className="object-cover transition duration-500 group-hover:scale-105" /></div><h3 className="line-clamp-2 text-[12px] font-semibold leading-[1.45] text-slate-800 transition group-hover:text-[#174a9b]">{post.title}</h3></Link>)}</div></aside>
    </div>
  </section>;
}
