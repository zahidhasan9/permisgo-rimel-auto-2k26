...
export default function MonitorFaqsPage() {
  ...
  useEffect(() => {
    if (!language) return;
    let active = true;
    getFaqs({ section: "instructors", lang: language }).then(({ data }) => {
      if (active) setFaqs(data?.data || []);
    });
    return () => {
      active = false;
    };
    ...
    () =>
      Object.entries(
        faqs.reduce((result, faq) => {
          const key = faq.category || "Instructor guidance";
          (result[key] ||= []).push(faq);
          return result;
        }, {}),
      ),
  ...
  return (
    <GuidePage
      eyebrow="Instructor help centre"
      title="Monitor FAQs"
      description="Clear answers to the questions our instructors and partners ask most often."
      icon={<FaQuestionCircle />}
      highlights={["Quick answers", "Instructor guidance", "Dedicated support"]}
