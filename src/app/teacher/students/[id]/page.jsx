...
export default function TeacherStudentDetailsPage() {
  ...
  const loadStudent = useCallback(async () => {
    ...
    try {
      ...
      setData(null);
      setError(
        getErrorMessage(requestError, "Student details could not be loaded."),
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading && !data) {
    return (
      <main className="min-h-screen bg-[#f8fafc] p-8 text-center text-sm font-semibold text-slate-500">
    ...
    return (
      <main className="min-h-screen bg-[#f8fafc] p-6">
        <button
          type="button"
          onClick={() => router.back()}
          className="rounded-xl bg-white px-4 py-2 font-bold text-[#16458f] shadow-sm"
        >
          Back
        </button>
        <div className="mx-auto mt-12 max-w-xl rounded-2xl border border-rose-200 bg-rose-50 p-6 text-center text-rose-700">
          {error}
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => router.back()}
            className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-[#16458f] shadow-sm"
          >
          ...
          <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="mt-1 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold capitalize text-emerald-700">
                  {student.status}
                </span>
              </div>
            </div>
            <div className="mt-5 space-y-3 text-sm text-slate-600">
              <p className="flex items-center gap-3">
                <FaEnvelope className="text-[#16458f]" />{" "}
                {student.email || "Not provided"}
              </p>
              <p className="flex items-center gap-3">
                <FaPhoneAlt className="text-[#16458f]" />{" "}
          ...
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {[
              ...
              >
                ...
                <p className="mt-2 text-2xl font-black text-[#16458f]">
                  {value}
                </p>
              </article>
            ))}
          ...
          <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            {progress ? (
              <div className="mt-4 space-y-3 text-sm text-slate-600">
                <p>
                  <strong className="block text-slate-900">Performance</strong>
                  <span className="capitalize">
                    {String(progress.performance || "not assessed").replaceAll(
                      "_",
                      " ",
                    )}
                  </span>
                ...
                <p>
                  ...
                  {progress.teacherNotes || "No notes"}
                </p>
                <p>
                  <strong className="block text-slate-900">
                    Next recommendation
                  </strong>
                  {progress.nextLessonRecommendation || "Not set"}
                </p>
              </div>
            ) : (
              <p className="mt-4 text-sm text-slate-500">
                No lesson report has been submitted yet.
              </p>
            )}
          </article>
          <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {(progress?.skillsCovered || []).length ? (
                  progress.skillsCovered.map((skill) => (
                    <span
                      key={skill}
              ...
              <p className="mt-2 text-sm text-slate-600">
                {progress?.areasToImprove?.join(", ") || "Not recorded."}
              </p>
            </div>
          </article>
        </section>
                  ...
                  lessons.map((lesson) => (
                    <tr key={lesson._id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 capitalize text-slate-600">
                        {String(
                          lesson.lessonProgress?.performance || "not assessed",
                        ).replaceAll("_", " ")}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link
                          ...
                          className="rounded-lg bg-[#16458f] px-3 py-2 text-xs font-bold text-white"
                        >
                          View lesson
                        </Link>
                      </td>
