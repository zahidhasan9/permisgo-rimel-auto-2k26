...
const StudentLogin = () => {
  const { content } = useCmsPageContent("student-login");
        ...
        <div className="rounded-[12px] bg-[#e7edf7] p-3 sm:p-[20px]" style={{ backgroundColor: settings.panelBackground || "#e7edf7" }}>
          ...
          <div className="min-h-[540px] rounded-[12px] bg-white px-5 py-5 sm:px-[20px] sm:py-[20px]">
            ...
            <form onSubmit={handleSubmit} className="mt-[54px]">
              ...
              <p className="mt-5 text-center text-[14px] text-[#52525b]">{copy("accountPrompt", "Have no account?")}{" "}<Link href={settings.registerButtonUrl || "/register/student"} className="font-semibold text-[#173d73] underline underline-offset-2" style={{ color: settings.registerButtonTextColor || "#173d73" }}>{copy("registerButton", "Register now")}</Link></p>
            </form>
          </div>
        </div>
      </div>
