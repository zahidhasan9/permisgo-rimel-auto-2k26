...
async function loadBlog(slug, locale) {
  const response = await fetch(
    `${apiUrl}/blogs/${encodeURIComponent(slug)}?lang=${locale}`,
    { next: { revalidate: 300 } },
  );
...
export async function generateMetadata({ params }) {
  ...
  const languages = Object.fromEntries(
    locales.map((lang) => [lang, `/${lang}/blogs/${post.slug}`]),
  );
  return {
    title: post.title,
...
export default async function LocalizedBlogPage({ params }) {
  ...
  const jsonLd = {
    ...
    inLanguage: locale,
    mainEntityOfPage: `${siteConfig.url}/${locale}/blogs/${post.slug}`,
  };
  return (
    <article className="mx-auto max-w-5xl px-5 py-12">
      ...
      />
      <p className="text-sm font-bold uppercase tracking-wider text-[#174a9b]">
        PermisGo Blog
      </p>
      ...
      <div
        className="blog-article mt-8 text-base leading-8 text-slate-700"
